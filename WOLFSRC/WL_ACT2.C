...
void SpawnStand (enemy_t which, int tilex, int tiley, int dir)
{
	...
	if (AM_GetMapTile(0, tilex, tiley) == AMBUSHTILE)
	{
		tilemap[tilex][tiley] = 0;

		if (AM_GetMapTile(0, tilex+1, tiley) >= AREATILE)
			tile = AM_GetMapTile(0, tilex+1, tiley);
		if (AM_GetMapTile(0, tilex, tiley-1) >= AREATILE)
			tile = AM_GetMapTile(0, tilex, tiley-1);
		if (AM_GetMapTile(0, tilex, tiley+1) >= AREATILE)
			tile = AM_GetMapTile(0, tilex, tiley+1);
		if (AM_GetMapTile(0, tilex-1, tiley) >= AREATILE)
			tile = AM_GetMapTile(0, tilex-1, tiley);

		AM_SetMapTile(0, tilex, tiley, tile);
